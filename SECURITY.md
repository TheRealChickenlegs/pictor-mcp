...
| Response headers | `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, `Cross-Origin-Resource-Policy: same-origin`, and `Cache-Control: no-store` on auth rejections. HSTS is opt-in (`PICTOR_ENABLE_HSTS`) and only meaningful behind TLS termination. The one exception is a served file under `/files/`, which is marked `Cross-Origin-Resource-Policy: cross-origin`: the whole point of a signed output URL is to be loaded by a page on another origin (Open WebUI's), and `same-origin` makes the browser discard exactly that request while the same URL keeps working in a new tab. |
...
| Signed output URLs | HMAC-SHA256 (128-bit truncated) over the path and expiry, constant-time verification, expiry enforced. The token travels in a path segment rather than a query string: a chat UI re-renders these URLs through markdown, HTML and a model's retyping, and a query string is what that pipeline drops. `/files/` is exempt from bearer auth *because* the signature is the credential — otherwise a browser `<img>` tag could never load a result. Serving is refused at startup unless a token or signing secret exists. |
...
| Access logging | uvicorn's per-request access log is **off** by default (`PICTOR_HTTP_ACCESS_LOG`), because a `/files/` URL carries its HMAC signature in the path and a logged line is therefore a usable credential. Application logging is unaffected. |
